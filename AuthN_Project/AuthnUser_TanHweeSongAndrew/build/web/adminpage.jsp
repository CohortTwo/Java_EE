...
<html>
    <head>
        ...
        <title>DB Administrator Page </title>
    </head>
    <body>
        ...
        <c:if test="${op == 'Delete'}">
           ...
           <c:if test="${op == 'Edit'}">
            <sql:update dataSource="${myDBAccess}" var="count">
                UPDATE HR.AUTH_TBL SET PASSWORD='<%=request.getParameter("pword")%>', USERNAME='<%=request.getParameter("usrnam")%>',INUSE=0  WHERE USERID='<%=request.getParameter("usrid")%>'
            </sql:update>        
        </c:if>    

                <% 
                    request.getRequestDispatcher("usrhome.jsp").forward(request, response);
                %>
    </body>
</html>   
